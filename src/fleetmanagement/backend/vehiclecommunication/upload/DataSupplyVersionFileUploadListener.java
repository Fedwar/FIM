...
@Component
public class DataSupplyVersionFileUploadListener extends VersionFileUploadListener {

	public DataSupplyVersionFileUploadListener(@Autowired VehicleRepository vehicles, @Autowired PackageSyncService packageSyncService) {
		super(vehicles, packageSyncService);
	}
	
	@Override
	public boolean canHandleUploadedFile(String filename) {
		return filename.equals("dv-status.json");
	}

	@DoNotObfuscate
	public class DataSupplyStatus implements DataSupplyVersionInterface{
		@SerializedName("data-supply-status")
